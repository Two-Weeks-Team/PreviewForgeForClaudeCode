...
5. **PreviewDD cache lookup** (v1.3+, v1.6.0에서 키 확장): profile.caching.preview_dd=true이면 `(idea_hash, idea_spec_hash, advocate_set_hash, model_version, profile.name)` 키로 `~/.claude/preview-forge/cache/preview-dd/` 조회. `idea_spec_hash`가 포함되어 동일 one-liner라도 Socratic 답변이 바뀌면 cache miss. hit이면 Advocate dispatch 스킵
...
11. 사용자 디자인 승인 후 SpecDD cycle 시작 (이후 `idea-drift-detector.py` 훅이 모든 spec write를 containment 검사 — v1.6.0부터 vocab은 채워진 `idea.spec.json` 필드까지 포함)
