...
4. **PreviewDD cache lookup** (v1.3+): profile.caching.preview_dd=true이면 `(idea_hash, advocate_set_hash, model_version, profile.name)` 키로 `~/.claude/preview-forge/cache/preview-dd/` 조회. hit이면 Advocate dispatch 스킵
5. I1 Idea Clarifier 호출 — 아이디어가 너무 모호하면 AskUserQuestion으로 한 번 정제 후 진행
6. I_LEAD가 **profile.previews.count명의 Advocate를 병렬 dispatch** (단일 메시지 N개 Task 호출). standard=9 · pro=18 · max=26. `--previews=N`으로 덮어쓰기 가능 (≤ max_user_expand)
...
10. `chosen_preview.json` + `mockups/chosen.html` 잠금 → Gate H1(`/pf:design`) 자동 호출
11. 사용자 디자인 승인 후 SpecDD cycle 시작 (이후 `idea-drift-detector.py` 훅이 모든 spec write를 containment 검사)
