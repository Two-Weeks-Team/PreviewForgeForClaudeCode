...
4. **Pre-Socratic weak-cache probe** (v1.6.1+ A-1 — one-click replay 복원): I1 실행 **전에** 동일 아이디어의 이전 run이 캐시에 있는지 경량 체크. 있으면 Socratic 3 모달을 사용자 선택으로 건너뛸 수 있어 "same idea 재실행 → 1-click" 서사가 복구된다.
   ...
   - **Hit**: AskUserQuestion 1개 — "이 아이디어의 이전 run이 캐시에 있습니다. Socratic 인터뷰를 건너뛰고 기존 previews를 재사용할까요?" `[Yes — 재사용 / No — Socratic 다시]`
     - **Yes** (weak-replay path):
       1. `cached`의 stdout은 이전 run의 `previews.json` 전체 내용(array) — 그대로 `runs/<id>/previews.json`에 기록
       2. `runs/<id>/idea.spec.json`에 schema-compliant stub 기록: `{"_schema_version":"1.0","_filled_ratio":0,"idea_summary":"<원본 idea 문자열>","_weak_replay":true,"_source_key":"<weak_key>"}`. 이렇게 두면 idea-clarifier가 "spec 존재 → I1 skip"으로 판단하고, I_LEAD가 "previews.json 이미 존재 → Advocate dispatch skip, I2 재검증만" 경로(기존 §6 cache hit과 동일한 코드 패스)를 탄다.
       3. §5(I1 Socratic)·§6(strong-key lookup)·§7(Advocate dispatch) 이 세 단계만 skip되고, §8(I2 diversity) 이후 panel·mitigation·Gate H1은 **정상 진행**된다. panel 재투표로 composite 추천이 원본 run과 달라질 수 있다는 점을 stdout에 한 줄 고지.
       4. user-facing modal 수: Yes/No(1) + Gate H1(1~2) = **2~3 modals** (fresh run: Socratic 3 + H1 1~2 = 4~5 modals). 엄밀한 one-click은 아니지만 Socratic 부담을 제거해 A-1 regression을 해소.
