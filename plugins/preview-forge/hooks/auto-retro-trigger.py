...
# S-6 (v1.7.0+) — bound run_id to a safe alphanumeric token so a hook-fed
# path like ``runs/../score/report.json`` cannot yield ``run_id=".."``
# which would then be ``Path.joinpath``'d into
# ``cwd/runs/../blackboard.db`` = ``cwd/blackboard.db`` — writing SQLite
# outside the intended run directory.
#
# The pattern is deliberately looser than the canonical
# ``r-YYYYMMDD-HHMMSS`` run-supervisor emits: CI fixtures and short-lived
# test runs use ``r-001`` / ``r-42`` / ``r-demo``, and future
# run-supervisor may append a suffix. Allowing ``[A-Za-z0-9_-]`` after a
# mandatory ``r-`` prefix + alphanumeric first char keeps every real
# run_id but still blocks every traversal / shell-meta string we care
# about — ``..``, ``.``, ``/``, ``;``, ``|``, ``$()``, backtick, space,
# newline, and unicode homoglyphs all fall outside the charset.
#
# Defense in depth: hook input is nominally trusted (comes from Claude
# Code), but sanitising here blocks agent write mishaps from escalating
# into filesystem writes above ``runs/<id>/``.
