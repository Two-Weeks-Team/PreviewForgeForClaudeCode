...
- Write: `runs/<id>/blackboard.db`, `runs/<id>/trace.jsonl`, `runs/<id>/trace.log` (v1.7.0+ D-4), `runs/<id>/status.json`
- Bash: **read-only pre-flight scripts + runs/<id> 부트스트랩만** 허용. 구체적으로:
  ...
  - **Run-dir bootstrap** (v1.7.0+ D-4): 딱 두 가지만 — (1) `mkdir -p "runs/r-<ts>"`로 run 디렉토리 생성, (2) `exec 2> >(tee -a "runs/<id>/trace.log" >&2)`로 stderr을 `trace.log`에 append-only tee. 그 외 `runs/<id>/` 내 파일 write는 위 Write 화이트리스트의 경로(`blackboard.db` / `trace.jsonl` / `trace.log` / `status.json`)에 한정되며, 이들은 Bash 대신 **Write·Edit tool**로 수행. 다른 run artifact(`previews.json`·`idea.json`·`chosen_preview.json` 등)는 각 department lead 책임이고 supervisor Bash에서 건드리지 않는다.
