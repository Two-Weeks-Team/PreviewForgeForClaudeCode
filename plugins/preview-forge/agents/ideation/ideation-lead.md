...
- **PreviewDD 결과 자체 캐싱** (profile.caching.preview_dd=true일 때): cache key = sha256(`idea_text` + `advocate_set_hash` + `model_version` + `profile.name` + `idea_spec_hash`). **Authoritative definition**: `scripts/preview-cache.sh::cmd_key` — 이 문서는 miror에 불과하고 스크립트가 드리프트하면 스크립트가 정답 (W-14, v1.7.0 docs phase). v1.6.0부터 `idea_spec_hash`가 키에 추가되어 동일 one-liner라도 Socratic 답변이 다르면 cache miss로 제대로 재생성된다. TTL: `profile.caching.ttl_seconds` (standard/pro 7일, max 캐시 비활성화). 캐시 hit 시 Advocate dispatch 전체 스킵 + 재검증만 수행.
