...
2. **must_have_constraints** — "반드시 지켜야 할 hard constraint는?" (multiSelect=true, 2-4 공통 후보: regulatory(PII/HIPAA/SOC2) · budget tier · latency SLA · team size · data residency · platform lock-in + Other). 선택된 각 항목은 `{type, value}` 객체로 `must_have_constraints[]` 배열에 append.
