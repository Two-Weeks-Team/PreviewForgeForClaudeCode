...
## 팀 구성 (profile-aware, v1.3+)

**standard (2 agents)**: `secqa-sast` + `secqa-secret-scan`
**pro (3 agents)**: 위 + `secqa-deps` (dependency audit)
**max (5 agents)**: 위 + `secqa-cors` + `secqa-authz` (권한/인가 테스트)
...
**Tools-vs-Agents 정책 (quality-engineer 패널 투표 결과)**:
- **도구 (semgrep, gitleaks, secretlint, OWASP ZAP)**: **profile 관계없이 항상 실행**. 도구는 load-bearing이므로 agent 수를 줄여도 도구는 유지
- **Agent**: profile 따라 축소 가능. standard는 1명의 sast agent가 semgrep + gitleaks 둘 다 오케스트레이션하는 식으로 압축
- 즉, standard 프로파일도 SAST 자체는 doing(도구로) 하지만 해석은 1명만 함
