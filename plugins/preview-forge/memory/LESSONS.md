...
- **참조**: `runs/r-20260422-184337/chosen_preview.panel-recommended.json` (P02 백업) vs `chosen_preview.json` (P19 override), v1.1.0 commit, 첫 실제 run 피드백 2026-04-22; v1.6.0+ I1 idea-clarifier (`agents/ideation/idea-clarifier.md`), v1.7.0 PR #51 (B-1/B-3/A-4), `schemas/idea-spec.schema.json`. **Regression fixtures**: `tests/fixtures/lesson07-regression/cases.json` (anchor invariant guard — Rule 9 anchors on chosen_preview, not composite_winner), `tests/fixtures/lesson07-regression/panel-bias-cases.json` + `simulate-panel-tally.py` (composite-scoring guard — W4.11b, #72; absorbs #78 N-6 LESSONS L61 fixture back-link).
